...
const UPI_ID = "9741813787@ybl";
const WHATSAPP_NUMBER = "917892099079";
const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbxwdgFrMXDiQn-MdnhsnP9E3t1wFPjBqpkkoxnbSqD4eT8p0bwfLCbXMvR9UxPeCI1wWQ/exec";
// ═════════════════════════════
...
function getDelivery() {
  return region === "karnataka"
    ? DELIVERY_KARNATAKA
    : DELIVERY_OTHER;
}

function getTotal() {
  return qty * BOOK_PRICE + getDelivery();
}
...
function updateBreakdown() {
  ...
  document.getElementById("pb_books").textContent = fmt(
    qty * BOOK_PRICE
  );
  document.getElementById("pb_region").textContent = regionLabel;
  document.getElementById("pb_delivery").textContent = fmt(
    getDelivery()
  );
...
function setRegion(r) {
  ...
  document
    .getElementById("btn-karnataka")
    .classList.toggle("selected", r === "karnataka");

  document
    .getElementById("btn-other")
    .classList.toggle("selected", r === "other");
...
function goToReview() {
  ...
  document.getElementById("rev_qty").textContent =
    qty +
    (lang === "kn"
      ? " ಪ್ರತಿ"
      : qty > 1
  ...
  document.getElementById("rev_books").textContent = fmt(
    qty * BOOK_PRICE
  );

  document.getElementById("rev_delivery").textContent =
    fmt(getDelivery()) + " (" + regionLabel + ")";
...
function goToPayment() {
  ...
  document.getElementById("pay_amount").innerHTML = fmt(getTotal());
...
function copyUPI() {
  navigator.clipboard.writeText(UPI_ID).then(() => {
    ...
    el.textContent =
      lang === "kn" ? "ನಕಲಿಸಲಾಗಿದೆ!" : "Copied!";

    setTimeout(() => {
      el.textContent = UPI_ID;
    }, 1500);
  });
}
...
function submitOrder() {
  ...
  fetch(GOOGLE_SCRIPT_URL, {
    ...
    document.getElementById("waLink").href =
      "https://wa.me/" +
      WHATSAPP_NUMBER +
      "?text=Hi%2C%20I%20placed%20an%20order%20%E2%80%94%20Ref%3A%20" +
